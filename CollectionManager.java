public class CollectionManager {
    ...
    // This is used so we dont have to pass Collections in the functions all the
    // time
    // Do this once at the start of the program

    // If we want to use different collections we have to bind another collection
    // set for our functions to operate on them

    public static void bind(Collection<Ad> ads, Collection<AdType> adTypes, Collection<AdAgency> agencies,
    ...
    public static AdType getAdTypeByTypeCode(String typeCode) {
        ...
        for (int i = 0; i < adTypes.getLength(); i++) {
            if (adTypes.get(i).getUniqueIdentifier().equals(typeCode))
                return adTypes.get(i);
        }
    ...
    public static void printAdsOf(AdAgency agency) {
        if (agency == null || adTypes.getLength() == 0 || ads.getLength() == 0) {
            System.out.printf("Agency is not null %b, AdTypes length %d, Ads length %d%n", agency, adTypes.getLength(),
                    ads.getLength());
            return;
        }
        ...
        for (int i = 0; i < m_ads.getLength(); i++) {
            ...
            for (int j = 0; j < adTypes.getLength(); j++) {
                ...
                if (curAd.getAdTypeCode().equals(curType.getUniqueIdentifier())) {
                    ...
                    int cost = curType.cost(curAd.getDetails(), curAd.getExtraCharacteristic(),
                            curAd.getDurationInDays());
    ...
    public static int printAdCostFor(AdAgency agency) {
        if (agency == null || adTypes.getLength() == 0 || ads.getLength() == 0) {
            // System.out.printf("Agency is not null %b, AdTypes length %d, Ads length
            // %d%n", agency, adTypes.getLength(), ads.getLength());
            return 0;
        }
    ...
    public static void printNumberOfAdsPerProduct() {
        ...
        for (int i = 0; i < countsOfAdsPerProduct.length; i++) {
            System.out.printf("Number of Ads for Product %s is : %d%n", productDescriptions[i],
                    countsOfAdsPerProduct[i]);
        }
    }
    ...
    public static int printAdCostFor(Product product) {
        ...
        if (product == null || adTypes.getLength() == 0 || ads.getLength() == 0) {
            // System.out.printf("Product is not null %b, AdTypes length %d, Ads length
            // %d%n", product, adTypes.getLength(), ads.getLength());
            return 0;
        }
    ...
    public static void printCostPerProduct() {
        if (products.getLength() == 0 || adTypes.getLength() == 0 || ads.getLength() == 0) {
            System.out.printf("Products length %d, AdTypes length %d, Ads length %d%n", products.getLength(),
                    adTypes.getLength(), ads.getLength());
            return;
        }
            ...
            for (int j = 0; j < ads.getLength(); j++) {
                ...
                if (curProduct.getProductCode().equals(curAd.getProductCode())) {
                    for (int k = 0; k < adTypes.getLength(); k++) {
                        ...
                        if (curType.getUniqueIdentifier().equals(curAd.getAdTypeCode())) {
                            costsPerProduct[i] += curType.cost(curAd.getDetails(), curAd.getExtraCharacteristic(),
                                    curAd.getDurationInDays());
                        }
                    }
                }
